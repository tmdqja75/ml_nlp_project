...
def getDict(df):
    ...
    try:
        ...
        pos_dict = pd.DataFrame({'pos' : pos,})
        nega_dict = pd.DataFrame({'nega' : nega,})
        neu_dict = pd.DataFrame({'neu' : neu,})
...
def getX(df):
    ...
    pos_dict, nega_dict, neu_dcit = getDict(df)

    try:
        cnt_df = pd.read_csv('cnt_df.csv', index_col=0)
